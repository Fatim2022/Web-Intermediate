...
export default class Map {
  ...
  addMarker(coordinates, markerOptions = {}, popupOptions = null) {
    ...
    // Tampilkan popup jika popupOptions disediakan
    if (popupOptions) {
      ...
      // Default behavior jika tidak ada popupOptions: tampilkan longlat
      newMarker.on('click', (event) => {
        const { lat, lng } = event.latlng;
        newMarker.bindPopup(`Latitude: ${lat.toFixed(5)}<br>Longitude: ${lng.toFixed(5)}`).openPopup();
      });
    }
